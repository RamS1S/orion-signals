"""
Orion Signals — Shared Styles
Κοινά CSS, background, logo για όλες τις σελίδες.
"""
...
.stApp{background:#020408;font-family:'Inter',sans-serif;color:#E2E8F0}
.block-container{position:relative;z-index:10;padding-top:0!important;max-width:520px!important}

.star-canvas{position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:0}
.orion-svg{position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:1;opacity:0.55}
.stApp::after{content:'';position:fixed;bottom:-100px;right:20%;width:500px;height:500px;
    background:radial-gradient(ellipse,rgba(124,58,237,0.07) 0%,transparent 70%);pointer-events:none;z-index:0}

.top-bar{position:fixed;top:0;left:0;right:0;height:52px;display:flex;align-items:center;
    padding:0 2rem;background:rgba(2,4,8,0.85);border-bottom:1px solid rgba(124,58,237,0.15);
    backdrop-filter:blur(12px);z-index:100}
.top-logo{font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800;color:#fff;letter-spacing:-0.02em}
.top-logo span{color:#7C3AED}
.top-spacer{height:72px}

.logo-wrap{text-align:center;margin-bottom:1.5rem}
.logo-icon{font-size:2.8rem;display:block;filter:drop-shadow(0 0 25px rgba(124,58,237,0.9));margin-bottom:0.2rem}
.logo-title{font-family:'Syne',sans-serif;font-size:2.4rem;font-weight:800;color:#fff;
    letter-spacing:-0.03em;margin:0;line-height:1}
.logo-title span{color:#7C3AED}
.logo-tagline{font-size:0.7rem;color:rgba(255,255,255,0.25);letter-spacing:0.28em;
    text-transform:uppercase;margin-top:0.35rem}

.stTabs [data-baseweb="tab-list"]{background:rgba(255,255,255,0.03)!important;
    border-radius:10px!important;padding:3px!important;gap:3px!important;
    border:1px solid rgba(124,58,237,0.15)!important;margin-bottom:1.2rem!important}
.stTabs [data-baseweb="tab"]{background:transparent!important;color:rgba(255,255,255,0.35)!important;
    border-radius:7px!important;font-family:'Syne',sans-serif!important;font-weight:700!important;
    font-size:0.82rem!important;letter-spacing:0.06em!important;padding:0.45rem 1.2rem!important;
    border:none!important;transition:all 0.2s!important}
.stTabs [aria-selected="true"]{background:rgba(124,58,237,0.28)!important;color:#fff!important;border:none!important}

.stTextInput>div>div>input{background:rgba(255,255,255,0.04)!important;
    border:1px solid rgba(124,58,237,0.22)!important;border-radius:8px!important;
    color:#fff!important;font-family:'Inter',sans-serif!important;font-size:0.82rem!important;
    padding:0.5rem 0.85rem!important;transition:all 0.2s!important;height:36px!important}
.stTextInput>div>div>input:focus{border-color:rgba(124,58,237,0.65)!important;
...
.stTextInput label{color:rgba(255,255,255,0.4)!important;font-size:0.7rem!important;
    font-weight:500!important;letter-spacing:0.12em!important;text-transform:uppercase!important}

.stButton>button{background:linear-gradient(135deg,#7C3AED 0%,#4F1D96 100%)!important;
    color:white!important;border:none!important;border-radius:8px!important;
    font-family:'Syne',sans-serif!important;font-weight:700!important;font-size:0.85rem!important;
    letter-spacing:0.08em!important;width:100%!important;padding:0.6rem!important;
    margin-top:0.2rem!important;transition:all 0.2s!important;
    box-shadow:0 4px 18px rgba(124,58,237,0.32)!important;height:40px!important}
.stButton>button:hover{transform:translateY(-1px)!important;
    box-shadow:0 6px 24px rgba(124,58,237,0.52)!important}

.phone-note{font-size:0.68rem;color:rgba(255,255,255,0.25);
    margin-top:-0.3rem;margin-bottom:0.3rem;padding-left:0.2rem}
.login-footer{text-align:center;margin-top:1.2rem;color:rgba(255,255,255,0.12);
    font-size:0.65rem;letter-spacing:0.05em}
.link-btn{background:none!important;border:none!important;color:rgba(124,58,237,0.8)!important;
    font-size:0.8rem!important;cursor:pointer;text-decoration:underline;width:auto!important;
    padding:0!important;height:auto!important;box-shadow:none!important;margin:0!important}
...
(function(){
    function init(){
        ...
        const stars=Array.from({length:220},()=>({
            x:Math.random(),y:Math.random(),r:Math.random()*1.1+0.15,
            base:Math.random()*0.4+0.08,speed:Math.random()*0.008+0.002,
...
<div class="top-bar"><div class="top-logo">🎯 Orion <span>Signals</span></div></div>
<div class="top-spacer"></div>
"""

def inject_background():
    st.markdown(ORION_CSS + ORION_BG, unsafe_allow_html=True)

def inject_logo(tagline=False):
    st.markdown(f"""
    <div class="logo-wrap">
        <span class="logo-icon">🎯</span>
        <h1 class="logo-title">Orion <span>Signals</span></h1>
        {'<p class="logo-tagline">See through the noise</p>' if tagline else ''}
    </div>
